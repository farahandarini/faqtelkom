...
<html>
    ...
    <body>
        ...
        <h1 style="text-transform: uppercase;">TAMBAH PERTANYAAN {{ $kat->kategori }}</h1>
        @endforeach
        ...
        <div class="container col-8">
            ...
            <form action="add_p2/add_tanyakategori" method="post">
                @csrf
                ...
                <div class="input-group input-group-sm mb-3">
                    {{-- <select id="kategori" name="kategori" class="form-select @error('kategori') is-invalid @enderror" aria-label="Default select example">
                        ...
                        @foreach($kategori as $kat)
                            <option value="{{ $kat->id }}" {{old('kategori')==$kat->id ? 'selected' : null}}>{{ $kat->kategori }}</option>
                        @endforeach
                    </select> --}}
                    @foreach($kategori as $kat)
                        <input type="text" name="kategori" id="kategori" class="form-control @error('kategori') is-invalid @enderror" value="{{ $kat->id }}" hidden>
                    @endforeach
                    @error('kategori')
                ...
                <div class="input-group input-group-sm mb-3">
                    <textarea id="text" name="pertanyaan" type="text" class="form-control @error('pertanyaan') is-invalid @enderror" aria-label="pertanyaan" aria-describedby="basic-addon1">{{ old('pertanyaan') }}</textarea>
                    @error('pertanyaan')
                        <span class="invalid-feedback" role="alert">
                            {{ $message }}
                        </span>
                ...
                <div class="input-group input-group-sm mb-3">
                    <textarea id="text" name="jawaban" type="text" class="form-control @error('jawaban') is-invalid @enderror jawab" aria-label="jawaban" aria-describedby="basic-addon1" rows="4">{{ old('jawaban') }}</textarea>
                    @error('jawaban')
                        <span class="invalid-feedback" role="alert">
                            {{ $message }}
                        </span>
                ...
                <div class="form-group">
                    <div class="d-flex justify-content-center">
                        <a type="button" class="btn cancel" href="{{URL::previous()}}">CANCEL</a>
